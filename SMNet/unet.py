...
class UpBlock(nn.Module):
    """
    A block consists of an upsampling layer followed by a convolutional layer to reduce the amount of channels and then a DownBlock
    ...
    def __init__(self, input_dim, output_dim, initializers, padding, bilinear=True):
        ...
        self.conv_block = DownBlock(input_dim, output_dim, initializers, padding, pool=False)
...
class UNet(nn.Module):
    ...
    def __init__(self, input_channels, num_classes, num_filters, initializers, apply_last_layer=True, padding=True):
        ...
        for i in range(len(self.num_filters)):
            ...
            self.contracting_path.append(DownBlock(input, output, initializers, padding, pool=pool))
        ...
        for i in range(n, -1, -1):
            ...
            output = self.num_filters[i]
            self.upsampling_path.append(UpBlock(input, output, initializers, padding))
